...
# Define the schema expected by LangChain
class AskHumanInput(BaseModel):
    question: str
    

# async def listen_to_coral_sse(url, agent_executor):
...
#                         logger.info(f"hehehe: {event_data}")
#                         try:
...
async def fetch_message(message_url: str):
    async with aiohttp.ClientSession() as session:
        async with session.get(f"http://localhost:5555{message_url}") as resp:
            if resp.status == 200:
                data = await resp.json()
                return data
            else:
                print(f"Failed to fetch message: {resp.status}")
                return None
            ...
            async for line in resp.content:
                if line:
                    ...
                    if decoded.startswith("data:"):
                        ...
                        message = await fetch_message(event_data)
